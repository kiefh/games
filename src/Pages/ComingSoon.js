...
function filterMapByComingSoon() {
    ...
    let parseTodaysDate = Date.parse(todayDate);
        ...
        else if (parseItemDate > parseTodaysDate){
            filteredGames.push(i);
        }
    }
        ...
        <div>
          ...
          <title>{ TITLE }</title>
            ...
            <Grid container spacing={3} >
                ...
                <Grid item xs={12} />
                <Grid item xs={12} >
                    <h2 className={classes.header}>Coming Soon</h2>
                </Grid>
                <Divider/>
...
function filterMapByGenre(filterGenreValue){
    let aMap = [];
    console.log(filterGenreValue);
    if(filterGenreValue === 'All Genres'){
        aMap = filteredGamesImmutable
    }
    else {
        for (let i of filteredGamesImmutable) {
            if (i.genre === filterGenreValue) {
                aMap.push(i);
            }
        }
    }
